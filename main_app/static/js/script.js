...
for (let i=0; i<true_answers.length; i++) {
    ...
    currentAns.forEach(option => {
        ...
        choice.setAttribute("style", "list-style: none")
...
btn.addEventListener('click', () => {
    ...
    if (percentage >= 80) {
        ...
        result.innerHTML = `You got ${percentage}% (${score}/${userAnswers.length}) correct - We need to hangout more!`
    } else if (percentage >= 40) {
        result.innerHTML = `You got ${percentage}% (${score}/${userAnswers.length}) correct - Oh boy... We really need to talk more...`
    } else if (percentage >= 20) {
        result.innerHTML = `You got ${percentage}% (${score}/${userAnswers.length}) correct - That's a little sad...`
    } else if (percentage >= 0) {
        result.innerHTML = `You got ${percentage}% (${score}/${userAnswers.length}) correct - Dang... Friendship Cancelled...`
    }
